...
app.use(
  ...
  rateLimit({
    ...
    legacyHeaders: false,
    message: {
      error: "تم تجاوز عدد الطلبات مؤقتًا، حاول بعد قليل."
    }
  })
);
  ...
  for (const item of knowledge) {
    for (const candidate of item.keys || []) {
      ...
      if (
        ...
        !level &&
        requestedLevel !== "P" &&
        /تحقيق|معنى|اعمل|اسوي/.test(normalize(q))
      ) {
        continue;
      }
      ...
      const levelBoost =
        requestedLevel && level === requestedLevel ? 4 : 0;

      const score =
        (a.length ? hit / a.length : 0) +
        exact +
        levelBoost;
...
const btecOnly = (q) => {
  ...
  return (
    ...
    (/كيف|شو|ما|هل/.test(n) &&
      /وحده|معيار|واجب|تخصص|دراسه|توجيهي|بيتک|بيتك/.test(n))
  );
};
...
app.post("/api/ask", async (req, res) => {
  try {
    ...
    /* 2) Criterion fallback */

    const requestedLevel =
      detectCriterionLevel(question);
    ...
    if (requestedLevel) {
      ...
      return res.json({
        ok: true,
        source: "قاعدة المعرفة الداخلية",
        answer: levelAnswers[requestedLevel],
        matchedQuestion:
    ...
    if (!client) {
      return res.status(503).json({
        ok: false,
        error:
          "خدمة Web Search + AI غير مفعلة. ضع مفتاح OpenAI الحقيقي في ملف .env ثم أعد تشغيل الـBackend."
      });
    }
...
بوابة BTEC الرسمية،
والجهات الرسمية أو Pearson عندما تكون ذات صلة.

إذا لم توجد إجابة موثوقة،
قل بوضوح إن المعلومات غير مؤكدة بدل اختلاقها.
...
const frontendCandidates = [
  // Backend inside a project folder
  path.resolve(
    __dirname,
    "..",
    "BTEC JO"
  ...
  // Frontend beside backend
  path.resolve(
    __dirname,
    "BTEC JO"
  ),

  // Current working directory + BTEC JO
  path.resolve(
    process.cwd(),
    "BTEC JO"
  ),
  ...
  // Frontend directly in current directory
  path.resolve(
    process.cwd()
  ),
...
if (!site) {
  console.error(
    "BTEC JO frontend not found. Checked:",
    frontendCandidates
  );
} else {
  console.log(
  ...
  if (
    ...
    if (!site) {
      return res
        .status(500)
        .send(
          "BTEC JO frontend is not available on the server."
        );
    }
